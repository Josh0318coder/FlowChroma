...
class UNetFusionNet(nn.Module):
    ...
    def __init__(self, in_channels=7, out_channels=2):
        ...
        # Merge blocks for skip connections (must be pre-defined, not created in forward)
        self.dec3_merge = self._conv_block(512, 256)  # 256 (up) + 256 (skip) = 512
        self.dec2_merge = self._conv_block(256, 128)  # 128 (up) + 128 (skip) = 256
        self.dec1_merge = self._conv_block(128, 64)   # 64 (up) + 64 (skip) = 128
    ...
    def forward(self, memflow_ab, swintexco_ab, memflow_conf, swintexco_sim, L_channel):
        ...
        d3 = self.dec3_merge(torch.cat([self.dec3(b), e3], dim=1))
        d2 = self.dec2_merge(torch.cat([self.dec2(d3), e2], dim=1))
        d1 = self.dec1_merge(torch.cat([self.dec1(d2), e1], dim=1))

        # Output
        residual = self.out_conv(d1)
        fused_ab = torch.tanh(memflow_ab + residual)
...
class FusionNetV1(nn.Module):
    ...
    def __init__(self, ic=8, delta_scale=0.3):
    ...
    def forward(self, memflow_lab, memflow_conf, swintexco_lab, swintexco_sim):
        """
        ...
        # Output delta AB (correction on top of SwinTExCo, bounded by delta_scale)
        pred_ab = self.conv10_ab(conv10_2)
        fused_ab = self.delta_scale * torch.tanh(pred_ab)
