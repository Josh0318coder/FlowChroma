...
parser.add_argument("--use_amp", action='store_true', default=True, help='Mixed precision training (bfloat16)')
parser.add_argument("--max_grad_norm", type=float, default=1.0, help='Max gradient norm for clipping')
parser.add_argument("--image_size", nargs='+', type=int, default=[224, 224])
parser.add_argument("--ic", type=int, default=8)
...
parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/M7")
...
def prepare_dataloader_simple(dataset, batch_size=4, pin_memory=True, num_workers=8):
    """簡單的數據載入器，無DDP支持"""
...
def load_checkpoint(checkpoint_dir):
    ...
    try:
        ...
        if os.path.exists(learning_state_path):
            ...
            if "scaler" in learning_checkpoint:
                scaler.load_state_dict(learning_checkpoint["scaler"])
...
if __name__ == "__main__":
    ...
    for epoch_num in range(start_epoch, opt.epoch+1):
        ...
        for iter, sample in enumerate(train_progress_bar):
            ...
            I_current_lab = I_current_lab.to(device, non_blocking=True)
            I_reference_lab = I_reference_lab.to(device, non_blocking=True)
            ...
            # Main forward pass in bfloat16
            with autocast(enabled=opt.use_amp, dtype=torch.bfloat16):
                with torch.no_grad():
            ...
            # Discriminator update (FP32, separate from generator scaler)
            discriminator_loss = torch.tensor(0.0, device=device)
            if opt.weight_gan > 0:
                fake_data_lab = torch.cat((
                    I_current_l, I_current_ab_predict.detach()
                ), dim=1)
            ...
            if epoch_num > opt.epoch_train_discriminator and opt.weight_gan > 0:
                fake_data_lab = torch.cat((I_current_l, I_current_ab_predict), dim=1)
                real_data_lab = torch.cat((I_current_l, I_current_ab), dim=1)
                generator_loss = generator_loss_fn(real_data_lab, fake_data_lab, discriminator, opt.weight_gan, device)

            # Perceptual + Contextual + Smoothness in FP32
            with autocast(enabled=False):
                I_predict_rgb = tensor_lab2rgb(torch.cat((uncenter_l(I_current_l), uncenter_ab(I_current_ab_predict.float())), dim=1))
                ...
                if abs(opt.weight_contextual) < 1e-8:
                    ...
                    contextual_style5_1 = torch.mean(contextual_forward_loss(pred_feat_3, B_feat_3.float().detach())) * 8
                    contextual_style4_1 = torch.mean(contextual_forward_loss(pred_feat_2, B_feat_2.float().detach())) * 4
                    contextual_style3_1 = torch.mean(contextual_forward_loss(pred_feat_1, B_feat_1.float().detach())) * 2
                    contextual_style2_1 = torch.mean(contextual_forward_loss(pred_feat_0, B_feat_0.float().detach()))
            ...
            if (iter + 1) % opt.accumulation_steps == 0 or (iter + 1) == len(data_loader):
                ...
                torch.nn.utils.clip_grad_norm_(
                    list(nonlocal_net.parameters()) + list(colornet.parameters()),
                    opt.max_grad_norm
                )
