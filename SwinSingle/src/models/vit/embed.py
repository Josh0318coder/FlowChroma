...
class SwinModel(nn.Module):
    ...
    def forward(self, x, return_raw_stage1=False):
        x = self.normalizer(x)
        outputs = self.pretrained(x)
        raw_stage1 = outputs[1]  # [B, 192, H/8, W/8] before upsampling — same format as MemFlow fnet
        outputs = [self.upsample(feat) for feat in outputs]
        if return_raw_stage1:
            return outputs, raw_stage1
        return outputs
