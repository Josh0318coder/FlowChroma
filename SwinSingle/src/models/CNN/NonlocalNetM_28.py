...
# ============ Up sample for concatenate ============       
        ...
        similarity_map = F.interpolate(sim_map.view(batch_size, 1, feature_height, feature_width), scale_factor=4)

        num_regions = indices_b_out.shape[1]
        K = indices_b_out.shape[2]
        B_lab_down = F.avg_pool2d(B_lab_map, 4).view(batch_size, 3, -1)
        idx_b = indices_b_out.view(batch_size, -1)
        B_colors = torch.gather(B_lab_down, 2, idx_b.unsqueeze(1).expand(-1, 3, -1))
        B_colors = B_colors.view(batch_size, 3, num_regions, K).permute(0, 2, 3, 1)

        sims_4d = similarities.view(batch_size, num_regions, K, K)
        weights = F.softmax(sims_4d / temperature, dim=-1)
        y_colors = torch.matmul(weights, B_colors)
        y_colors = y_colors.view(batch_size, num_regions * K, 3)

        y_out = torch.zeros(batch_size, feature_height * feature_width, 3, device=device)
        idx_a = indices_a_out.view(batch_size, -1)
