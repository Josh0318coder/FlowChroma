...
def mapping_to_gray(B_lab, f, temperature=0.005):
    """
    輸入:
    - B_lab: [batch_size, 3, 224, 224] - reference image
    - f: [batch_size, 3136, 3136] - cosine similarity matrix, values in [-1, 1]
    - temperature: softmax temperature

    輸出:
    - y: [batch_size, 3, 224, 224] - mapped color
    - similarity_map: [batch_size, 1, 224, 224] - similarity map
    """
    batch_size = B_lab.shape[0]

    # Similarity map: max cosine similarity per A position, normalized to [0, 1]
    similarity_map = (torch.max(f, dim=-1)[0] + 1) / 2  # [B, 3136]
    similarity_map = F.interpolate(
        similarity_map.view(batch_size, 1, 56, 56), scale_factor=4)  # [B, 1, 224, 224]

    # Softmax color transfer
    B_lab_down = F.avg_pool2d(B_lab, 4).view(batch_size, 3, -1).permute(0, 2, 1)  # [B, 3136, 3]
    f_div_C = F.softmax(f / temperature, dim=-1)  # [B, 3136, 3136]
    y = torch.matmul(f_div_C, B_lab_down)  # [B, 3136, 3]
    y = y.permute(0, 2, 1).contiguous().view(batch_size, 3, 56, 56)
    y = F.interpolate(y, scale_factor=4)  # [B, 3, 224, 224]
...
# =======================================================
        ...
        y, similarity_map = mapping_to_gray(B_lab_map, f, temperature=temperature)
