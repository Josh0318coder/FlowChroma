...
class WarpNet(nn.Module):
    ...
    def precompute_B_features(self, B_lab_map, B_relu2_1, B_relu3_1, B_relu4_1, B_relu5_1):
        """Pre-compute reference (B-side) features. Call once per reference frame and cache the result."""
    ...
    # Internal timing flag — set to True from outside to print per-stage breakdown
    _profile_internals = False
    ...
    def forward(
        ...
        # A-side features (must recompute every frame)
        ...
        if self._profile_internals:
            t1 = _evt()  # after A layer2_1~5_1

        if cached_B_features is not None:
            # Use pre-computed B-side features (skip redundant B-side convolutions)
            phi, B_lab = cached_B_features
            ...
            # B-side features (computed fresh when no cache available)
            B_feature2_1 = self.layer2_1(B_relu2_1)
            B_feature3_1 = self.layer3_1(B_relu3_1)
            B_feature4_1 = self.layer4_1(B_relu4_1)
            B_feature5_1 = self.layer5_1(B_relu5_1)
        ...
        if self._profile_internals:
            t2 = _evt()  # after A self.layer (ResBlocks)
        ...
        if self._profile_internals:
            t3 = _evt()  # after matmul (theta × phi)
        ...
        # Max cosine similarity as confidence, normalized to [0, 1]
        ...
        if self._profile_internals:
            ...
            a_conv  = t0.elapsed_time(t1)
            resblk  = t1.elapsed_time(t2)
            matmul1 = t2.elapsed_time(t3)
            rest    = t3.elapsed_time(t4)
            print(f"[WarpNet internals]  A-conv:{a_conv:.2f}ms  ResBlocks:{resblk:.2f}ms  "
                  f"matmul(θ×φ):{matmul1:.2f}ms  rest:{rest:.2f}ms  "
                  f"total:{a_conv+resblk+matmul1+rest:.2f}ms")
