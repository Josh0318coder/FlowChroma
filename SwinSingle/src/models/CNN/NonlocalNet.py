...
class WarpNet(nn.Module):
    ...
    def __init__(self, feature_channel=128):
        ...
        # Removed: self.upsampling = nn.Upsample(scale_factor=4)
        # Output now stays at 56×56 (H/4) to match MemFlow downsample output
    ...
    def forward(
        ...
        similarity_map = (similarity_map + 1) / 2  # Map [-1, 1] to [0, 1]
        ...
        y = y.view(batch_size, channel, feature_height, feature_width)  # 2*3*56*56 (H/4)
        # Removed upsampling: output stays at 56×56 to match MemFlow downsample output
        # y = self.upsampling(y)
