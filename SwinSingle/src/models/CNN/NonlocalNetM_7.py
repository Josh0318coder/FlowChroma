...
# ============ Up sample for concatenate ============       
        ...
        similarity_map = F.interpolate(
            sim_map.view(batch_size, 1, feature_height, feature_width), scale_factor=4)
        ...
        y_out = torch.zeros(batch_size, feature_height * feature_width, 3, device=device)
        idx_a = indices_a_out.view(batch_size, -1)  # [B, 3136]
        y_out.scatter_(1, idx_a.unsqueeze(-1).expand(-1, -1, 3), y_colors)
        y = y_out.permute(0, 2, 1).view(batch_size, 3, feature_height, feature_width)
        y = F.interpolate(y, scale_factor=4)
