...
def warp_color(
    ...
    nonlocal_net,
    temperature=0.01,
    features_A=None,  # optimization ①: pre-computed target features (skip embed_net call)
    cached_B_features=None,  # optimization ②: pre-computed WarpNet B-side (phi, B_lab)
):
    if features_A is not None:
        A_feat0, A_feat1, A_feat2, A_feat3 = features_A
    else:
    ...
    return nonlocal_net(
        ...
        cached_B_features=cached_B_features,
    )
