...
def frame_colorization(
    ...
    with torch.autograd.set_grad_enabled(joint_training):
        ...
        zeros_conf = torch.zeros_like(similarity_map)  # proxy for memflow_conf (no MemFlow in newsingle)
        delta_ab = colornet(
            torch.cat(
                (IA_l, nonlocal_BA_ab, zeros_conf,        # memflow_lab(3) + memflow_conf(1)
                 IA_l, nonlocal_BA_ab, similarity_map),   # swintexco_lab(3) + swintexco_sim(1)
                dim=1,
            )
        )
        IA_ab_predict = torch.tanh(nonlocal_BA_ab + delta_ab)
