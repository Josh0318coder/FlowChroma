...
# ============ Up sample for concatenate ============       
        ...
        # Similarity map: normalize to [0, 1]
        sim_map = (max_similarities + 1) / 2  # [B, 3136]
        sim_map_raw = sim_map.view(batch_size, 1, feature_height, feature_width)
        similarity_map = F.interpolate(sim_map_raw, scale_factor=4)

        # Softmax color transfer with correct global reference positions
        num_regions = indices_b_out.shape[1]  # 49
        K = indices_b_out.shape[2]            # 64
        B_lab_down = F.avg_pool2d(B_lab_map, 4).view(batch_size, 3, -1)  # [B, 3, 3136]
        idx_b = indices_b_out.view(batch_size, -1)                        # [B, 49*64]
        B_colors = torch.gather(B_lab_down, 2, idx_b.unsqueeze(1).expand(-1, 3, -1))  # [B, 3, 3136]
        B_colors = B_colors.view(batch_size, 3, num_regions, K).permute(0, 2, 3, 1)   # [B, 49, 64, 3]

        sims_4d = similarities.view(batch_size, num_regions, K, K)  # [B, 49, 64_A, 64_B]
        weights = F.softmax(sims_4d / temperature, dim=-1)           # [B, 49, 64_A, 64_B]
        y_colors = torch.matmul(weights, B_colors)                   # [B, 49, 64_A, 3]
        y_colors = y_colors.view(batch_size, num_regions * K, 3)     # [B, 3136, 3]

        # Scatter to correct A positions
        y_out = torch.zeros(batch_size, feature_height * feature_width, 3, device=device, dtype=y_colors.dtype)
        idx_a = indices_a_out.view(batch_size, -1)  # [B, 3136]
        y_out.scatter_(1, idx_a.unsqueeze(-1).expand(-1, -1, 3), y_colors)
        y_raw = y_out.permute(0, 2, 1).view(batch_size, 3, feature_height, feature_width)
        y = F.interpolate(y_raw, scale_factor=4)

        if return_raw:
            return y, similarity_map, y_raw, sim_map_raw
        return y, similarity_map
