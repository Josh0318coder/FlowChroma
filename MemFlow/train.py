...
def train(gpu, cfg):
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            # ===== 載入數據 =====
            images = data_blob['images'].cuda()        # [B, 4, 3, H, W] LAB
            ...
            with torch.cuda.amp.autocast(enabled=cfg.mixed_precision, dtype=torch.bfloat16):
                # Encode context (前3幀) - 使用彩色RGB（與 RAFT_Color 一致）
                query, key, net, inp = model.module.encode_context(rgb_color_inputs[:, :-1, ...])

                # Encode features (所有4幀) - 使用灰階RGB（與 RAFT_Color 一致）
