...
class MemFlowNet(nn.Module):
    ...
    def upsample_flow(self, flow, mask):
        """ Upsample flow field [H/8, W/8, 2] -> [H/4, W/4, 2] using convex combination (2x upsample) """
        N, _, H, W = flow.shape
        mask = mask.view(N, 1, 9, 2, 2, H, W)  # Changed from 8x8 to 2x2
        mask = torch.softmax(mask, dim=2)

        up_flow = F.unfold(2 * flow, [3, 3], padding=1)  # Changed from 8 to 2
        up_flow = up_flow.view(N, 2, 9, 1, 1, H, W)

        up_flow = torch.sum(mask * up_flow, dim=2)
        up_flow = up_flow.permute(0, 1, 4, 2, 5, 3)
        return up_flow.reshape(N, 2, 2 * H, 2 * W)  # Changed from 8x to 2x
