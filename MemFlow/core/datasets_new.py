...
class VideoColorizationDataset(Dataset):
    ...
    def _load_and_process_image(self, path):
        ...
        Returns:
            rgb_gray:  [3, H, W] - ImageNet標準化的灰階RGB（給 fnet）
            rgb_color: [3, H, W] - ImageNet標準化的彩色RGB（給 cnet）
        ...
        try:
            ...
            # ===== 2. 處理 彩色 RGB（給 cnet） =====
            rgb_color = self.to_tensor(image)          # [3, H, W], [0,1]
            rgb_color = self.normalize(rgb_color)      # ImageNet標準化
            ...
            return rgb_gray, rgb_color, lab

        except Exception as e:
            print(f"❌ Error loading {path}: {e}")
            # 返回默認值
            rgb_gray = torch.zeros(3, self.image_size[0], self.image_size[1])
            rgb_color = torch.zeros(3, self.image_size[0], self.image_size[1])
            lab = torch.zeros(3, self.image_size[0], self.image_size[1])
            lab[0] = 50.0
            return rgb_gray, rgb_color, lab

    def _apply_augmentation(self, rgb_list, rgb_color_list, lab_list):
        """
        同步數據增強 - 應用到所有4幀

        Args:
            rgb_list:       list of [3, H, W] - 灰階RGB
            rgb_color_list: list of [3, H, W] - 彩色RGB
            lab_list:       list of [3, H, W] - LAB
        """
        if not self.augment:
            return rgb_list, rgb_color_list, lab_list

        # 水平翻轉（同步所有幀）
        if random.random() > 0.5:
            rgb_list = [torch.flip(rgb, [-1]) for rgb in rgb_list]
            rgb_color_list = [torch.flip(rgb_c, [-1]) for rgb_c in rgb_color_list]
    ...
    def __getitem__(self, idx):
        ...
        for frame_path in seq_info['frame_paths']:
            rgb, rgb_color, lab = self._load_and_process_image(frame_path)
            rgb_list.append(rgb)
            rgb_color_list.append(rgb_color)
            lab_list.append(lab)

        # 數據增強（同步所有幀）
        rgb_list, rgb_color_list, lab_list = self._apply_augmentation(rgb_list, rgb_color_list, lab_list)

        # Stack成序列格式
        rgb_seq = torch.stack(rgb_list, dim=0)        # [4, 3, H, W] - 灰階RGB
        rgb_color_seq = torch.stack(rgb_color_list, dim=0)  # [4, 3, H, W] - 彩色RGB
        lab_seq = torch.stack(lab_list, dim=0)        # [4, 3, H, W] - LAB

        return {
            'images': lab_seq,              # [4, 3, H, W] - LAB序列（用於 loss）
            'rgb_inputs': rgb_seq,          # [4, 3, H, W] - 灰階RGB（給 fnet）
            'rgb_color_inputs': rgb_color_seq,  # [4, 3, H, W] - 彩色RGB（給 cnet）
