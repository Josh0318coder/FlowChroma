...
def train_epoch(system, dataloader, criterion, optimizer, scaler, epoch, args, discriminator=None, optimizer_d=None):
    ...
    for batch_idx, batch_data in enumerate(pbar):
        ...
        reference_pil_batch = batch_data['reference_pil']  # List[PIL.Image], one per sequence
        ...
        for seq_idx in range(batch_size):
            frames_lab = frames_lab_batch[seq_idx]  # List of LAB tensors
            frames_pil = frames_pil_batch[seq_idx]  # List of PIL images
            reference_pil = reference_pil_batch[seq_idx]  # Single PIL reference (shared across sequence)
                    ...
                    if i == 0 and discriminator is not None and args.weight_gan > 0:
                        ...
                        with autocast(enabled=False):
                            ...
                            if epoch > args.epoch_train_discriminator:
                                ...
                                # Backward immediately to avoid SpectralNorm version mismatch
        ...
        if (batch_idx + 1) % args.accumulation_steps == 0:
            ...
            scaler.update()
            optimizer.zero_grad()
            # Note: Discriminator is updated immediately after its backward (see GAN training section)
...
def main():
    ...
    parser.add_argument('--weight_gan', type=float, default=0.1,
