...
class AdaptiveTemporalLoss(nn.Module):
    ...
    def forward(self, fusion_t, fusion_t1, memflow_t, memflow_t1,
        ...
        Args:
            fusion_t: [B, 2, H, W] Fusion output AB channels at frame t (full resolution)
            fusion_t1: [B, 2, H, W] Fusion output AB channels at frame t+1 (full resolution)
            memflow_t: [B, 2, H/4, W/4] MemFlow output AB channels at frame t (56×56)
            memflow_t1: [B, 2, H/4, W/4] MemFlow output AB channels at frame t+1 (56×56)
            memflow_conf_t: [B, 1, H/4, W/4] MemFlow confidence at frame t (56×56)
            memflow_conf_t1: [B, 1, H/4, W/4] MemFlow confidence at frame t+1 (56×56)

        Returns:
            loss: scalar
        """
        # Upsample memflow outputs to match fusion resolution (56 → 224)
        H, W = fusion_t.shape[2:]
        if memflow_t.shape[2:] != (H, W):
            memflow_t = F.interpolate(memflow_t, size=(H, W), mode='bilinear', align_corners=True)
            memflow_t1 = F.interpolate(memflow_t1, size=(H, W), mode='bilinear', align_corners=True)
            memflow_conf_t = F.interpolate(memflow_conf_t, size=(H, W), mode='bilinear', align_corners=True)
            memflow_conf_t1 = F.interpolate(memflow_conf_t1, size=(H, W), mode='bilinear', align_corners=True)
