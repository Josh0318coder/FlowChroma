...
class FusionSystem(nn.Module):
    ...
    def memflow_inference(self, frame_t, frame_t1):
        ...
        with torch.no_grad(), autocast(enabled=True):
            ...
            # Memory management: use most recent frames
            if self.memflow_keys is None:
                # First frame (curr_ti=0): no historical references
                ref_values = None
                ref_keys = key.unsqueeze(2)
            else:
                # Subsequent frames: use all stored historical frames + current frame
            ...
            # Update memory: keep most recent (num_ref_frames - 1) frames
            if self.memflow_keys is None:
                ...
                # Keep only the most recent (num_ref_frames - 1) frames
                # This prevents unbounded memory growth in long sequences
                max_history = self.num_ref_frames - 1
                if updated_keys.shape[2] > max_history:
                    # Keep the last max_history frames (most recent)
                    self.memflow_keys = updated_keys[:, :, -max_history:]
                    self.memflow_values = updated_values[:, :, -max_history:]
                else:
