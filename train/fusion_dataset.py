...
class FusionSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        self.target_size = target_size
        self.swintexco_processor = swintexco_processor
    ...
    def __getitem__(self, idx):
        """
        Get a sequence of frames with a shared reference
        ...
        Returns:
            Dictionary containing:
                ...
                - reference_pil: Single PIL Image (shared across entire sequence)
                - video_name: str
        """
        ...
        frames_lab = []
        frames_swintexco_lab = []

        # Select reference image ONCE for the entire sequence (use first frame's ref_list)
        first_row = self.annotations.iloc[indices[0]]
        first_source_path = first_row.get('_source_path', None)

        if np.random.random() < self.real_reference_probability:
            # Use ImageNet external reference (from first frame's candidates)
            ref_list = [str(first_row['ref1']), str(first_row['ref2']), str(first_row['ref3']),
                        str(first_row['ref4']), str(first_row['ref5'])]
            reference = self._select_reference(ref_list)
        else:
            # Use video first frame
            first_frame_name = self.annotations[
                self.annotations['video_name'].astype(str) == video_name
            ].iloc[0]['current_frame']
            reference = self._load_frame(video_name, str(first_frame_name), first_source_path)

        reference_resized = reference.resize(self.target_size[::-1], Image.LANCZOS)
        reference_swintexco_lab = self.swintexco_processor(reference_resized)
        ...
        for idx in indices:
            ...
            # Convert to LAB (for MemFlow / loss computation)
            frame_lab = self._rgb_to_lab_tensor(frame_pil_resized)
            frames_lab.append(frame_lab)

            # Convert to SwinTExCo format (pre-processed in DataLoader worker)
            frames_swintexco_lab.append(self.swintexco_processor(frame_pil_resized))

        return {
            'frames_lab': frames_lab,                      # List of [3,H,W] LAB tensors
            'frames_swintexco_lab': frames_swintexco_lab,  # List of [3,H,W] SwinTExCo tensors
            'reference_swintexco_lab': reference_swintexco_lab,  # [3,H,W] SwinTExCo tensor
            'video_name': video_name
        }
...
def fusion_sequence_collate_fn(batch):
    ...
    Returns:
        Dictionary containing:
            ...
            - reference_pil: List[PIL.Image] - batch_size (single reference per sequence)
            - video_names: List[str]
    """
    return {
        'frames_lab': [item['frames_lab'] for item in batch],
        'frames_swintexco_lab': [item['frames_swintexco_lab'] for item in batch],
        'reference_swintexco_lab': [item['reference_swintexco_lab'] for item in batch],
